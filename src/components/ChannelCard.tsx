...
export default function ChannelCard({ channel, variant = 'default', index = 0 }: ChannelCardProps) {
  ...
  if (variant === 'compact') {
    return (
      <Link
        ...
        className={`flex items-center gap-3 p-3 glass-hover rounded-xl group fade-slide-up ${staggerClass}`}
      >
        ...
        {channel.isLive && (
          <span className="flex items-center gap-1.5 px-2 py-1 bg-danger/10 rounded-full border border-danger/20">
  ...
  if (variant === 'featured') {
    return (
      <Link
        href={`/watch/${channel.id}`}
        className={`channel-card animated-border relative group overflow-hidden rounded-2xl glass glow-pulse-hover fade-slide-up ${staggerClass}`}
      >
        ...
        {/* Info */}
        <div className="p-4 bg-background-secondary/80">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="text-sm text-foreground-secondary mt-0.5">
                {channel.country} &bull; {channel.language}
              </p>
            </div>
            {channel.isLive && (
              <span className="flex items-center gap-1.5 px-2 py-1 bg-danger/10 rounded-full border border-danger/20 flex-shrink-0">
  ...
  return (
    <Link
      href={`/watch/${channel.id}`}
      className={`channel-card animated-border group overflow-hidden rounded-xl glass glow-pulse-hover fade-slide-up ${staggerClass}`}
    >
      {/* Inner card content */}
      <div className="bg-background-secondary/60 rounded-xl overflow-hidden">
        {/* Logo Container */}
        <div className="relative aspect-video bg-gradient-to-br from-background-tertiary to-background p-6 flex items-center justify-center">
          ...
          {channel.isLive && (
            <span className="absolute top-2 left-2 flex items-center gap-1 px-1.5 py-0.5 bg-danger/90 rounded">
              <span className="w-1.5 h-1.5 bg-white rounded-full live-pulse-ring" />
